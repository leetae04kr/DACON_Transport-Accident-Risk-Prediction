...
def main():
    ...
    # 모델 및 교정기 경로 정의 (2개 Classifier 모델만)
    ...
    processed_chunks_A = []
    try: # ★ 여기가 오류 발생 지점 근처 ★ (들여쓰기 및 리스트 컴프리헨션 확인)
        ...
        # processed_chunks_A는 이미 빈 리스트로 초기화됨
    gc.collect()
    ...
    try:
        ...
        # processed_chunks_B는 이미 빈 리스트로 초기화됨
    gc.collect()
    ...
    # test.csv 병합 + Lag/Static PK/Hypothesis 피처 생성 (타겟 인코딩 제외)
    try:
    ...
    # ★ 2개 모델 최적 가중치 적용 (Score 0.146011 당시) ★
    WEIGHT_LGB_CLF = 0.3939
    WEIGHT_CAT_CLF = 0.6061 # 1.0 - WEIGHT_LGB_CLF
    print(f"Applying final optimized 2-model weights (LGB_clf: {WEIGHT_LGB_CLF:.4f}, Cat: {WEIGHT_CAT_CLF:.4f})...")
